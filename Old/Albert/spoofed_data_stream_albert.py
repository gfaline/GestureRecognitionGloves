...
class RealtimeSensorStreamer:
    ...
    def _should_interrupt(self):
        max_windows = random.randint(2, 4)
    ...
    def stream(self):
        while True:
            ...
            for gesture in self.gesture_chunks:
                gesture_label = gesture.iloc[0]['gestureTarget']
                # noise first
                ...
                for _, row in gesture.iterrows():
                    ...
                    if prev_t is not None:
                        dt = (data['timestamp_ms'] - prev_t) / 1000.0
                        if dt <= 0 or dt > 1:
                            dt = 0.05
                    else:
                        dt = 0.05
...
class SlidingWindow:
    ...
    def add(self, sample):
        ...
        self.label_buffer.append(sample['gestureTarget'])
        self.button_buffer.append(sample['active'])
...
if __name__ == "__main__":
    ...
    streamer = RealtimeSensorStreamer("albert_filtered.csv")
    window = SlidingWindow(WINDOW_SIZE, STRIDE)

    scaler_filename = 'alb_win_gesture_scaler1.pkl'
    model_filename = 'alb_win_forest_gesture_model1.pkl'
    try:
